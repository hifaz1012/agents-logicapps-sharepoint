...
class AzureLogicAppTool:
    ...
    def invoke_logic_app(self, logic_app_name: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(f"Invoking Logic App '{logic_app_name}' at URL: {url} with payload: {payload}")
        response = requests.post(url=url, json=payload)

        if response.ok:
            return response.json() if response.content else {"status": "Logic App invoked successfully."}
        else:
            return {"error": (f"Error invoking {logic_app_name} " f"({response.status_code}): {response.text}")}


def invoke_logicapps_sharepoint(service: AzureLogicAppTool, logic_app_name: str) -> Callable[[str], str]:
    """
    ...
    def send_http_request_via_logic_app_sharepoint(query: str) -> str:
        """
    ...
    return send_http_request_via_logic_app_sharepoint